...
async function run() {
  try {
    ...
    const fruitCollection = client.db('fruit_brust').collection('all_fruit');
    const cartCollection = client.db('fruit_brust').collection('carts');

    app.get('/all_fruit', async (req, res) => {
    ...
    app.get('/fruit/:id', async (req, res) => {
      ...
      const result = await fruitCollection.findOne(query, options);
      res.send(result);
    })
...
app.get('/', (req, res) => {
  res.send('Welcome to Fruit bistro')
})
